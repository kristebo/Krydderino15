...
RELEASE_LINE  := $(shell curl -s $(RELEASE_PATH) | grep -e '>release=' | cut -d'>' -f 2 | cut -d'<' -f 1 | cut -d'=' -f 2 | sed 's/&bull;/•/g')
...
prepare:
		@if [ -f Utilities/embedXcode_prepare ]; then Utilities/embedXcode_prepare $(PROJECT_FILE_PATH); rm -r Utilities/embedXcode_prepare; if [ $(RELEASE_LAST) ]; then if [ $(RELEASE_LAST) -gt $(RELEASE) ]; then osascript -e 'tell application "System Events" to if button returned of (display dialog "A new release $(RELEASE_LAST) is available.\n$(RELEASE_DATE) release $(RELEASE_LAST) • $(RELEASE_TEXT)\n\nInstalled release is $(RELEASE)." buttons {"Go to Download", "OK"} with icon POSIX file ("$(UTILITIES_PATH)/TemplateIcon.icns") default button 2 with title "embedXcode" giving up after 5) = "Go to Download" then tell application "Safari"' -e 'open location "http://www.embedXcode.weebly.com/download"' -e 'activate' -e 'end tell'; fi; fi; fi;
		@if [ $(RELEASE_LAST) ]; then if [ $(RELEASE_LAST) -gt $(RELEASE) ]; then echo "==== New release $(RELEASE_LAST) available ===="; echo "$(RELEASE_DATE) release $(RELEASE_LAST) • $(RELEASE_TEXT)"; echo "==== Release $(RELEASE) installed ===="; fi; fi;
